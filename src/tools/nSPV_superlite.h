...
char *NSPV_expand_variables(char *bigbuf,char *filestr,char *method,cJSON *argjson)
{
    ...
    else if ( strcmp(method,"send_confirm") == 0 || strcmp(method,"send_validate") == 0 )
    {
        ...
        if ( dest != 0 && satoshis != 0 )
        {
            ...
            if ( satoshis > (NSPV_utxosresult.balance+NSPV_utxosresult.interest-10000) )
                satoshis = (NSPV_utxosresult.balance+NSPV_utxosresult.interest-10000);
            fprintf(stderr,"%.8f vs %.8f\n",dstr(satoshis),dstr(NSPV_utxosresult.balance+NSPV_utxosresult.interest-10000));
            sprintf(replacestr,"%.8f",dstr(satoshis));
